...
class UniProtAPIClient:
    ...
    def get_pdb_uniprot_mapping(self, pdb_id: str, uniprot_id: str) -> Optional[Dict]:
        """获取PDB到UniProt的残基范围映射"""
        try:
            cache_query = f"mapping_{pdb_id}_{uniprot_id}"
            cached = self._get_cached(cache_query)
            if cached:
                return cached
            
            logger.info(f"获取PDB到UniProt残基映射: {pdb_id} -> {uniprot_id}")
            ...
            if response.status_code == 200:
                ...
                if pdb_lower in data:
                    mappings = data[pdb_lower]
                    for uni_id, mapping_data in mappings.items():
                        if uni_id.upper() == uniprot_id.upper():
                            # 找到对应的UniProt ID
                            residues = mapping_data.get('residues', [])
                            if residues:
                                starts = [r.get('start', 0) for r in residues]
                                ends = [r.get('end', 0) for r in residues]
                                result = {
                                    'residue_range': f"{min(starts)}-{max(ends)}",
                                    'uniprot_start': min(starts),
                                    'uniprot_end': max(ends)
                                }
                                self._save_cache(cache_query, result)
                                return result
                            
                            # 如果没有residues，尝试从 chains 获取
                            chains = mapping_data.get('chains', [])
                            for chain in chains:
                                chain_residues = chain.get('residues', [])
                                if chain_residues:
                                    starts = [r.get('start', 0) for r in chain_residues]
                                    ends = [r.get('end', 0) for r in chain_residues]
                                    result = {
                                        'residue_range': f"{min(starts)}-{max(ends)}",
                                        'uniprot_start': min(starts),
                                        'uniprot_end': max(ends)
            ...
            logger.warning(f"未找到PDB {pdb_id} 到 {uniprot_id} 的映射")
            return None
            
        except Exception as e:
            logger.error(f"获取PDB映射异常: {e}")
            return None
